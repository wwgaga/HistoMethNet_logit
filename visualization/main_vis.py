...
ckpt_path = '/cbica/home/tianyu/projects/dna-rrt-patch-only/results/task_dna_subtyping_RRT_NIH_and_Penn_Ken_classifier_s1/s_9_checkpoint_best_r.pt'
...
csv_path = '../dataset_csv/dna_methylation_upenn_dana_suggest.csv'  # Update this path as needed
...
for slide_id in slide_ids:
    ...
    mus.savefig(f'ken_new_ckpts/precentage_maps/percentages_{slide_id}.jpg', format='JPEG')

    # Store the percentages in a CSV file
    pd.DataFrame(percentages, columns=[f'{cell_type_names[i]}' for i in range(percentages.shape[1])]).to_csv(f'ken_new_ckpts/precentage_csv/cell_type_percentages_{slide_id}.csv', index=False)

    # Save the heatmap
    cat_map.save(f'ken_new_ckpts/categorical_maps/cell_type_heatmap_{slide_id}.jpg', 'JPEG')
